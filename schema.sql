...
-- edit rack capacity, add sites), admin (everything a manager
-- can do, plus remove sites).
-- ============================================================
...
-- restrict direct deletes to admins, same as site deletion
drop policy if exists "racks_delete_managers" on racks;
create policy "racks_delete_admin" on racks
  for delete using (public.is_admin());
...
create table if not exists rack_events (
  id uuid primary key default gen_random_uuid(),
  rack_id text not null references racks(id) on delete cascade,
  event_type text not null check (event_type in ('device_added','device_removed','capacity_changed')),
...
begin
  ...
  if (tg_op = 'INSERT') then
    ...
    values (new.rack_id, 'device_added',
      new.model || ' — ' || new.actual_kw || ' kW (U' || new.start_u ||
    ...
    values (old.rack_id, 'device_removed',
      old.model || ' — ' || old.actual_kw || ' kW (U' || old.start_u ||
...
-- auto-log rack capacity changes
create or replace function public.log_capacity_change()
returns trigger as $$
declare
  actor_email text;
begin
...
create trigger on_rack_capacity_change
  after update on racks
  for each row execute procedure public.log_capacity_change();

-- ============================================================
-- After running this file:
-- 1. Sign up your first user from the app's login screen.
-- 2. In Table Editor -> profiles, change that user's `role` to 'admin'
--    (admins can do everything managers can, plus remove sites).
-- 3. Promote other accounts to 'manager' the same way as needed.
-- 4. Everyone else who signs up defaults to 'technician' (read-only).
--
-- If you're upgrading an EXISTING project that already ran the old
